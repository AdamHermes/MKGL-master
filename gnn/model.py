...
class ConditionedPNA(PNA):
    ...
    def init_input_embeds(self, x, h_emb, h_idx, t_emb, t_idx, rel_emb, batch):
        ...
        Args:
            x: (B*N, dim) repeated node features
            h_emb: (batch_size, dim) head embeddings
            h_idx: (batch_size,) head node indices in repeated graph (1D!)
            t_emb: (batch_size, dim) tail embeddings
            t_idx: (batch_size,) tail node indices in repeated graph (1D!)
            rel_emb: (batch_size, rel_dim) relation embeddings
            batch: (B*N,) node->graph mapping
        """
        x = x.clone()
        
        # h_idx and t_idx should be 1D after indexing [:, 0]
        # If they're still 2D, take first column
        if h_idx.dim() > 1:
            h_idx = h_idx[:, 0]
        if t_idx.dim() > 1:
            t_idx = t_idx[:, 0]
    ...
    def forward(self, h_index, r_index, t_index,
        ...
        num_nodes = graph.num_nodes  # Use actual number of nodes in graph
        ...
        tail_embs = score_text_embs[tail_orig_idx].to(x_rep.dtype)  # Map original indices to embeddings

        # 7) Initialize input embeddings
        # Note: h_index and t_index are already offset to repeated graph
        # Just take the first column (positive sample) and ensure they're 1D
        h_idx_flat = h_index[:, 0]  # (batch_size,) 
        t_idx_flat = t_index[:, 0]  # (batch_size,)
